...
class MediaContent extends StatelessWidget {
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Row(
                children: [
                  Text('Carpetas multimedia', style: Theme.of(context).textTheme.headlineSmall),
  ...
  Padding _buildEmptyAnimationWidget(BuildContext context) {
    return Padding(
      ...
      child: TAnimationLoaderWidget(
        width: 300,
        height: 300,
        text: 'Seleccione su carpeta deseada',
  ...
  List<ImageModel> _getSelectedFolderImages(MediaController controller) {
    ...
    } else if (controller.selectedPath.value == MediaCategory.marcas) {
      images = controller.allBrandImages.where((image) => image.url.isNotEmpty).toList();
    } else if (controller.selectedPath.value == MediaCategory.categorias) {
      images = controller.allCategoryImages.where((image) => image.url.isNotEmpty).toList();
    } else if (controller.selectedPath.value == MediaCategory.productos) {
      images = controller.allProductImages.where((image) => image.url.isNotEmpty).toList();
    } else if (controller.selectedPath.value == MediaCategory.usuarios) {
  ...
  Widget buildAddSelectedImagesButton() {
    return Row(
      ...
      children: [
        ...
        SizedBox(
          width: 120,
          child: OutlinedButton.icon(label: const Text('Cerrar'), icon: const Icon(Iconsax.close_circle), onPressed: () => Get.back()),
        ),
        const SizedBox(width: TSizes.spaceBtwItems),
        SizedBox(
          width: 120,
